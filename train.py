...
def dropout(x, pdrop, train):
    if train and pdrop > 0:
        x = tf.compat.v1.nn.dropout(x, 1-pdrop)
    return x
...
def split_heads(x, n, k=False):
    if k:
        return tf.compat.v1.transpose(split_states(x, n), [0, 2, 3, 1])
    else:
        return tf.compat.v1.transpose(split_states(x, n), [0, 2, 1, 3])

def merge_heads(x):
    return merge_states(tf.compat.v1.transpose(x, [0, 2, 1, 3]))

def conv1d(x, scope, nf, rf, w_init=tf.compat.v1.random_normal_initializer(stddev=0.02), b_init=tf.compat.v1.constant_initializer(0), pad='VALID', train=False):
    with tf.compat.v1.variable_scope(scope):
        nx = shape_list(x)[-1]
...
def clf(x, ny, w_init=tf.compat.v1.random_normal_initializer(stddev=0.02), b_init=tf.compat.v1.constant_initializer(0), train=False):
    with tf.compat.v1.variable_scope('clf'):
        nx = shape_list(x)[-1]
...
def model(X, M, Y, train=False, reuse=False):
    with tf.compat.v1.variable_scope('model', reuse=reuse):
        we = tf.compat.v1.get_variable("we", [n_vocab+n_special+n_ctx, n_embd], initializer=tf.compat.v1.random_normal_initializer(stddev=0.02))
        ...
        if train and clf_pdrop > 0:
            ...
            shape[1] = 1
            clf_h = tf.compat.v1.nn.dropout(clf_h, 1-clf_pdrop, shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_records', type=int, default=0)    # for faster debugging the conversion for TF2 
    ...
    (trX1, trX2, trX3, trY), (vaX1, vaX2, vaX3, vaY), (teX1, teX2, teX3) = encode_dataset(rocstories(data_dir, num_records=num_records), encoder=text_encoder)
    n_y = 2
    ...
    X_train = tf.compat.v1.placeholder(tf.int32, [n_batch_train, 2, n_ctx, 2])
    M_train = tf.compat.v1.placeholder(tf.float32, [n_batch_train, 2, n_ctx])
    X = tf.compat.v1.placeholder(tf.int32, [None, 2, n_ctx, 2])
    M = tf.compat.v1.placeholder(tf.float32, [None, 2, n_ctx])

    Y_train = tf.compat.v1.placeholder(tf.int32, [n_batch_train])
    Y = tf.compat.v1.placeholder(tf.int32, [None])

    train, logits, clf_losses, lm_losses = mgpu_train(X_train, M_train, Y_train)
    clf_loss = tf.compat.v1.reduce_mean(clf_losses)
    ...
    eval_clf_loss = tf.compat.v1.reduce_mean(eval_clf_losses)
    eval_mgpu_clf_loss = tf.compat.v1.reduce_mean(eval_mgpu_clf_losses)
